...
export default async function AuditDashboardPage({ params }: PageProps) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-10">
      <section className="flex flex-col gap-6 rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-slate-100">Granskning #{audit.id.slice(0, 8)}</h1>
            <p className="mt-2 text-sm text-slate-400">
              Rot-URL: <span className="font-medium text-slate-200">{audit.rootUrl}</span>
            </p>
          </div>
        ...
        <dl className="grid gap-4 md:grid-cols-4">
          ...
          <SummaryTile
            ...
            value={audit.pages.reduce((sum, page) => sum + page.issues.length, 0).toString()}
          />
          <SummaryTile label="AI-sammanfattning" value={summary ? "Tillgänglig" : "Ej klar"} />
        </dl>
      </section>
      ...
      {summary ? (
        <section className="grid gap-6 lg:grid-cols-[2fr,1fr]">
          <article className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-xl font-semibold text-slate-100">Översikt</h2>
            <p className="text-sm leading-relaxed text-slate-300 whitespace-pre-line">{summary.text}</p>
          </article>
          <article className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-xl font-semibold text-slate-100">Snabba vinster</h2>
            {Array.isArray(summary.payload?.quick_wins) ? (
              <ul className="space-y-3 text-sm text-slate-300">
                {summary.payload.quick_wins.map(
                  (win: { title: string; description: string }, index: number) => (
                    <li key={`${win.title}-${index}`}>
                      <p className="font-semibold text-slate-100">{win.title}</p>
                      <p className="text-slate-400">{win.description}</p>
                    </li>
                  )
                )}
              </ul>
            ) : (
              <p className="text-sm text-slate-400">Analysen är på väg.</p>
            )}
          </article>
        </section>
      ) : null}
      ...
      <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-xl font-semibold text-slate-100">Sidor</h2>
          ...
          <table className="min-w-full divide-y divide-slate-800 text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              {audit.pages.map((page) => (
                <tr key={page.id} className="text-slate-300">
...
function ArtifactLinks({
  ...
  if (links.length === 0) {
    return <span className="text-xs text-slate-500">Klar när sidan har bearbetats.</span>;
  }
